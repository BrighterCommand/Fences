...
[`.agent_instructions/adr_frontmatter.md`](https://github.com/BrighterCommand/Fences/blob/main/.agent_instructions/adr_frontmatter.md).
...
| `0003-upstream-polly-sync-process` | Upstream Polly Sync Process | Accepted | meta, licensing, governance | Every upstream sync from App-vNext/Polly runs a mandatory licence-text gate before any diff, triage or port step; ported commits rebrand to Fences namespaces while preserving Polly's public API shape, with any unavoidable incompatibility called out at the point it occurs and in the tracking issue; a read-only upstream remote and polly-upstream tracking branch bound each sync's diff to what changed since the last one, and dependency-bump commits are skipped by default. |
