...
public class CircuitBreakerOpenedBenchmark
{
    ...
    public void Setup()
    {
        _reactivePipeline = (ResiliencePipeline?)Helper.CreateOpenedCircuitBreaker(FencesVersion.V8, handleOutcome: true);
        _pipeline = (ResiliencePipeline?)Helper.CreateOpenedCircuitBreaker(FencesVersion.V8, handleOutcome: false);
        _policy = (IAsyncPolicy<string>?)Helper.CreateOpenedCircuitBreaker(FencesVersion.V7, handleOutcome: false);
    }
