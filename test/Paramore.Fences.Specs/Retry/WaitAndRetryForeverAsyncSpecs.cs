using Scenario = Paramore.Fences.Specs.Helpers.PolicyExtensionsAsync.ExceptionAndOrCancellationScenario;

namespace Paramore.Fences.Specs.Retry;
...
public class WaitAndRetryForeverAsyncSpecs : IDisposable
{
    ...
    public void Should_throw_when_sleep_duration_provider_is_null_without_context()
    {
        ...
        Action<Exception, TimeSpan, Context> onRetryTimeSpanContext = (_, _, _) => { };

        policy = () => Policy
            .Handle<DivideByZeroException>()
            .WaitAndRetryForeverAsync(null, onRetryTimeSpanContext);
        ...
        Func<int, Exception, Context, TimeSpan> sleepDurationProviderKitchenSink = null!;

        policy = () => Policy
            .Handle<DivideByZeroException>()
            .WaitAndRetryForeverAsync(sleepDurationProviderKitchenSink, (_, _, _) => TaskHelper.EmptyTask);
        ...
        policy = () => Policy
            .Handle<DivideByZeroException>()
            .WaitAndRetryForeverAsync(sleepDurationProviderKitchenSink, (_, _, _, _) => TaskHelper.EmptyTask);
    ...
    public void Should_throw_when_onretryasync_exception_int_timespan_is_null_with_sleep_duration_provider()
    {
        ...
        Func<Exception, int, TimeSpan, Context, Task> onRetryAsync = null!;

        policy = () => Policy
            .Handle<Exception>()
            .WaitAndRetryForeverAsync((_, _, _) => TimeSpan.Zero, onRetryAsync);
    ...
    public async Task Should_wait_asynchronously_for_async_onretry_delegate()
    {
        ...
        // If Paramore.Fences were to declare only an Action<...> delegate for onRetry - but users declared async () => { } onRetry delegates - the compiler would happily assign them to the Action<...>, but the next 'try' would/could occur before onRetry execution had completed.
