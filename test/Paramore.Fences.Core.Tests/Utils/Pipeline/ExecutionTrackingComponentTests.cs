...
public class ExecutionTrackingComponentTests
{
    ...
    public async Task DisposeAsync_PendingOperations_Delayed()
    {
        ...
        var execution = Task.Run(() => new ResiliencePipeline(component, Paramore.Fences.Utils.DisposeBehavior.Allow, null).Execute(() => { }), TestCancellation.Token);
        executing.WaitOne();
    ...
    public async Task DisposeAsync_WhenRunningMultipleTasks_Ok()
    {
        ...
        var pipeline = new ResiliencePipeline(component, Paramore.Fences.Utils.DisposeBehavior.Allow, null);
