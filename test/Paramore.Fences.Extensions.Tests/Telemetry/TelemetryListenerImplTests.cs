...
public class TelemetryListenerImplTests : IDisposable
{
    ...
    public void WriteEvent_EnsureSeverityRespected(ResilienceEventSeverity severity, LogLevel logLevel)
    {
        ...
        var events = GetEvents("resilience.fences.strategy.events");
    ...
    public void WriteExecutionAttemptEvent_Metering_Ok(bool noOutcome, bool exception)
    {
        ...
        var events = GetEvents("resilience.fences.strategy.attempt.duration");
        events.Count.ShouldBe(1);
        var ev = events[0];
        ...
        _events.Single(v => v.Name == "resilience.fences.strategy.attempt.duration").Measurement.ShouldBe(50000);
    }

    [Fact]
    public void WriteExecutionAttemptEvent_ShouldBeSkipped()
    {
        using var metering = TestUtilities.EnableFencesMetering(_events, _ => false);
    ...
    public void WriteEvent_MeteringWithoutBuilderInstance_Ok()
    {
        ...
        var telemetry = Create();
        ReportEvent(telemetry, null, instanceName: null);
        GetEvents("resilience.fences.strategy.events")[0].ShouldNotContainKey("pipeline.instance");
    }
    ...
    public void PipelineExecution_Metered(bool exception)
    {
        ...
        var ev = _events.Single(v => v.Name == "resilience.fences.pipeline.duration").Tags;
    ...
    public void PipelineExecuted_ShouldBeSkipped()
    {
        ...
        var events = GetEvents("resilience.fences.pipeline.duration");
        events.ShouldBeEmpty();
    }
    ...
    // Only capture metering events emitted by this test's pipeline, as the "Paramore.Fences" meter is process-wide
    // and other tests running concurrently can emit events into the same listener.
    private IDisposable EnableMetering() =>
        TestUtilities.EnableFencesMetering(_events, shouldRecord: TestUtilities.ForPipelines(PipelineName));
