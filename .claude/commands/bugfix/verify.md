...
# Run the suite to confirm the fix holds and no regressions; prepare commit/PR
...
1. Run the regression test(s) and confirm they pass:
   `dotnet test {test-project} --framework net10.0 --filter "FullyQualifiedName~{TestName}"`
2. Run the broader suite for the touched project(s) to check for regressions:
   `dotnet test {test-project}`
   There is no external infrastructure in this repository — every test runs in process, so there is
   nothing to stand up first.
3. **Analyser and lint gates.** The build treats warnings as errors, so confirm the build is clean.
   If anything under `src/Snippets` changed, run `dotnet mdsnippets` and commit the regenerated
   Markdown. If a public member changed, confirm the project's
   `.PublicAPI/PublicAPI.Unshipped.txt` was updated.
4. **Mutation score.** If the fix touched a project under `src/`, run that project's mutation
   target (`./build.ps1 -Target MutationTestsCore` and friends) and confirm the score has not
   regressed. State the number.
5. **Report results faithfully.** If anything fails, say so with the output and stop — do not
