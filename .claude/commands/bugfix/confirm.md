...
# Confirm the root-cause hypothesis before any fix (the ✋ Confirm gate)
...
until this passes. Confirming the theory frequently changes the *scope* of the fix — proving that
a retry was firing on a cancelled outcome may surface a second defect in how the outcome was
classified, which the suggested fix only partially addressed.
