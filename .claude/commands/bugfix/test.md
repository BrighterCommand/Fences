...
# Write the failing regression test for a confirmed bug (delegates to /test-first)
...
- 🔴 RED: write the test per `.agent_instructions/testing.md` conventions
  (`Member_Scenario_Outcome`, grouped by type under test, Arrange/Act/Assert, Evident Data,
  `FakeTimeProvider` for time, public exports only) and run it to confirm it fails for the right reason.
