...
# Make the regression test green with the minimal change scoped to the confirmed cause
...
2. Follow `.agent_instructions/code_style.md` (`PascalCase` constants, required expression-bodied
   members, `TimeProvider` instead of `DateTime.Now`, **no licence header**) and
   `.agent_instructions/documentation.md` (XML docs on everything a package exports).
...
4. **Public API**: if the fix changed anything `public` or `protected` in `src/`, append the entry
   to that project's `.PublicAPI/PublicAPI.Unshipped.txt` — see
   `.agent_instructions/public_api.md`. A fix that needs an API change usually means the scope was
   wrong; say so rather than widening it silently.
5. **Multi-targeting**: the shipping libraries target down to `netstandard2.0` and `net462`. Check
   any API you reach for is available there, or guard it.
