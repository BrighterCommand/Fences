...
filename stem** (`0037-hedging-result-selection`), which is unique; all tooling keys off that.
...
```text
User: /adr backoff curve selection

[Claude scans docs/adr/ and finds highest is 0036]
Next ADR number: 0037

[Claude checks for current spec and finds specs/0001-adaptive-retry-backoff/]
...
> The retry strategy needs to widen its delay when the dependency is already saturated, without a caller hand-tuning the delay generator for every pipeline...
...
[Claude creates docs/adr/0037-hedging-result-selection.md]
Created: docs/adr/0037-hedging-result-selection.md
...
The ADR has been added to specs/0001-adaptive-retry-backoff/.adr-list
```
