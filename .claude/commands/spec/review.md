...
# Review current specification phase (requirements, design, tasks, or code)
...
- Extract **base-ref**: for `code` phase, a token starting with `--base=` (e.g., `--base=origin/main`). Default base is `main`.
...
- `/spec:review code` → review branch diff vs. main, threshold 60
- `/spec:review code 70` → review branch diff vs. main, threshold 70
...
- Resolve `{base-ref}` (default `main`). Run `git rev-parse --verify {base-ref}` — if it fails, tell the user the base ref doesn't exist and stop.
...
**TDD Compliance (Fences-specific):**
...
- Test naming: `Member_Scenario_Outcome`. Class name: `[TypeUnderTest]Tests`. Violations are Low–Medium unless pervasive.
- **Critical Fences rule**: a bug fix MUST come with a test that fails without the fix. A fix commit with no accompanying test is a High finding.
- **Critical Fences rule**: no real `Task.Delay` to wait for a strategy. Timing is controlled with `FakeTimeProvider`; a test that sleeps is flaky by construction and is a High finding.
...
- For anything on the execution path: what does it allocate? The engine sits inside other people's request paths, and `bench/` exists to answer this.
...
- Secrets or credentials committed to code, logs, or test fixtures
- Unsafe deserialization of attacker-controlled data
- `Random` used where a cryptographically secure source is required. Note that retry jitter is *not* such a case — do not file that as a finding
- New telemetry or logging that could emit caller data (exception messages, result values, context properties) into a log or trace
- New reflection or dynamic code generation, which is both an AOT problem and a trust-boundary problem

**Project Conventions (read `.agent_instructions/code_style.md` yourself to verify):**

- Constants are `PascalCase`, not `ALL_CAPS`. No licence header region on any file.
- Expression-bodied members are required at severity `error` — a block body where an expression body would do fails the build.
...
- XML doc comments on everything a package exports (StyleCop requires them)
- `using` directives outside the namespace, `System.*` first; file-scoped namespaces
...
**Public API discipline (Fences-specific, and the most common miss):**

- Any new or changed `public`/`protected` member in `src/` must have a matching entry in that project's `.PublicAPI/PublicAPI.Unshipped.txt`. A missing entry fails the build; an entry added to `Shipped.txt` by hand is a High finding.
- A public API change that the issue did not ask for is a finding in its own right, even if the entry is correct. The default posture is not to change the surface.
- A removal must appear as a `*REMOVED*` line, not as a deletion from `Shipped.txt`.

**Multi-targeting and compatibility (Fences-specific):**

- `Paramore.Fences.Core`, `.Extensions` and `.RateLimiting` target `net8.0;net6.0;netstandard2.0;net472;net462`. Flag any API used that is not available on `netstandard2.0`/`net462` without a polyfill in `src/LegacySupport` or a conditional compilation guard.
- `src/Paramore.Fences` is the frozen legacy package. A new feature there is a High finding; so is enabling nullable reference types on it.
- Anything reflective, dynamic, or generically unbounded added to `src/` should be checked against `test/Paramore.Fences.AotTest`.

**Strategy design (Fences-specific):**

- A new strategy is either reactive or proactive, has a matching `*Options` class, and declares handled outcomes through `PredicateBuilder`. Deviation without an ADR is a finding.
- `ResilienceContext` is pooled — flag any reference to one captured beyond its execution.
- Analyser and lint cleanliness: the build treats warnings as errors, so a StyleCop, Sonar or BannedApi warning introduced by the diff is a finding. `DateTime.Now` and friends are banned; use `TimeProvider`.
- If `src/Snippets` changed, `mdsnippets` must have been re-run and the regenerated Markdown committed alongside it.
- If a project under `src/` was touched, its Stryker mutation score must not have regressed.
