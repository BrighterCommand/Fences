...
- [ ] **TEST + IMPLEMENT: [Behavior description]**
  ...
  - Test file: `[Member_Scenario_Outcome.cs]`
  - Test should verify:
...
- [ ] **TEST + IMPLEMENT: Rejection with no channels configured acknowledges message**
  ...
  - Test location: "test/Paramore.Fences.Core.Tests/Retry"
  - Test file: `RetryResilienceStrategyTests.cs`
  - Test should verify:
    - Consumer created without backoff or invalid message routing keys
    - Message rejected with DeliveryError reason
    - Message acknowledged (can consume next message)
    - Warning logged
  - **⛔ STOP HERE - WAIT FOR USER APPROVAL in IDE before implementing**
  - Implementation should:
    - In RetryResilienceStrategy.Reject() check if both producers are null
