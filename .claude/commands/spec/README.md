...
```bash
/spec:new adaptive-retry-backoff
```

Creates:

- `specs/NNNN-adaptive-retry-backoff/` directory
...
Active Spec: specs/0001-adaptive-retry-backoff/ (marked with *)

* specs/0001-adaptive-retry-backoff/
  Issue: #123

  Requirements: ✓ Approved

  Design (ADRs):
    - docs/adr/0042-adaptive-retry-backoff-jitter.md [Accepted]
    - docs/adr/0043-adaptive-retry-backoff-errors.md [Proposed]
    - docs/adr/0044-adaptive-retry-backoff-state.md [Accepted]
...
2. Writes a failing test following [.agent_instructions/testing.md](../../../.agent_instructions/testing.md):
   ...
   - File per test: `Member_Scenario_Outcome.cs`
   ...
   - Controls time with `FakeTimeProvider`; no real `Task.Delay`
...
- [ ] **[Brief behavior description]**
  ...
  - **Test file**: `test/[Project]/[Member_Scenario_Outcome.cs]`
  - **Test should verify**:
    - [Point 1]
    - [Point 2]
  - **Implementation files**:
    - `src/[Project]/[File.cs]` - [What to add/change]
  - **RALPH-VERIFY**: `dotnet test test/[Project]/ --filter "FullyQualifiedName~Member_Scenario_Outcome"`
...
### Example: Adaptive Retry Backoff Feature

Instead of one large ADR covering everything, create focused ADRs:

1. **ADR 0042: Message Serialization** - How to serialize/deserialize messages for backoff
2. **ADR 0043: Error Classification** - How to determine which errors send to backoff
3. **ADR 0044: Persistence Strategy** - How to store backoff metadata
4. **ADR 0045: Retry Mechanism** - How to handle retries from backoff
...
- The first commit should include the ADR(s)
- A draft PR with the ADRs allows early feedback
- Follow TDD practices; a bug fix must include a test that fails without the fix
- Do not change the public API unless the issue asks for it, and record any change you do make in
  `src/<Project>/.PublicAPI/PublicAPI.Unshipped.txt`
- Do not add or update dependencies unless the issue asks for it
- Target `main`; use Conventional Commits
...
- See [Fences Documentation](https://brightercommand.github.io/Fences/)
