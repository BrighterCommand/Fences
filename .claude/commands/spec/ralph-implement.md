...
- Test naming: `Member_Scenario_Outcome`; group tests by type under test in a file named
  the same; Arrange/Act/Assert with explicit comments; highlight evident data.
- Test behavior, not implementation — public exports only. Control time with `FakeTimeProvider`;
  never introduce a real `Task.Delay`. Prefer the helpers in `test/Paramore.Fences.TestUtils`.
...
- Code style: .NET C# conventions (PascalCase public / camelCase private), `PascalCase`
  constants (**not** `ALL_CAPS`), expression-bodied members (**required**, at severity
  `error`), `readonly` where appropriate, `TimeProvider` instead of `DateTime.Now`, no licence
  header. If the public API changed, update the project's `.PublicAPI/PublicAPI.Unshipped.txt`.
